"""Independent training entry for Experiment H decoder-LFP ablations."""
...
OUTPUT_NAMES = {
    "h1_rawll_attention": "H1_rawll_attention",
    "h1_decoder_attention": "H1_decoder_attention",
    "h2_rawll_fixed_gaussian": "H2_rawll_fixed_gaussian",
    "h2_decoder_fixed_gaussian": "H2_decoder_fixed_gaussian",
    "h3_rawll_adaptive_gaussian": "H3_rawll_adaptive_gaussian",
    "h3_decoder_adaptive_gaussian": "H3_decoder_adaptive_gaussian",
}


def parse_args():
    parser = argparse.ArgumentParser(description="Train Experiment H decoder LFP")
    ...
    parser.add_argument(
        "--lfp-variant",
        required=True,
        choices=EXPERIMENT_H_VARIANTS[1:],
        help="H0 is regression/profile only and must not be formally trained",
    )
...
def apply_variant_metadata(args):
    config = DECODER_LFP_VARIANT_CONFIGS[args.lfp_variant]
    args.ablation_id = config["ablation_id"]
    args.model_variant = f"dwtfreqnet_e1_{args.lfp_variant}"
    args.sd_variant = args.lfp_variant
    args.encoder_variant = "e1_lfss_resblock"


def build_model(args, mode):
    return DWTFreqNet_SingleDecoder_LFSS_AWGM_DecoderLFP(
...
def evaluate(model, loader, device, threshold):
    ...
    statistics = getattr(model, "last_lfp_statistics", None)
    if statistics:
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Experiment H training")
    set_seed(args.seed)
    ...
    for stage in range(1, 5):
        ...
        if processor.use_gaussian and abs(float(processor.gaussian.sigma.detach()) - 1.0) > 1e-6:
            raise RuntimeError("Experiment H Gaussian sigma initialization is not 1.0")
    ...
    run_config = {
        ...
        **model.experiment_metadata,
        ...
        "lfp_parameters": parameter_count(model, ("decoder_lfp",)),
        "attention_parameters": parameter_count(
            model, tuple(f"decoder_lfp{stage}.attention" for stage in range(1, 5))
        ),
        "gaussian_parameters": parameter_count(
            model, tuple(f"decoder_lfp{stage}.gaussian" for stage in range(1, 5))
        ),
        "adaptive_threshold_parameters": parameter_count(
            model, tuple(f"decoder_lfp{stage}.threshold_predictor" for stage in range(1, 5))
        ),
        "dwt_calls": 4,
        "idwt_calls": 4,
