...
def open_image_mask(dataset_dir, sample_name):
    """Open a pair, including NUAA masks named with the _pixels0 suffix."""
    image_path = None
    mask_path = None
    extensions = ('.png', '.bmp', '.jpg', '.jpeg', '.tif', '.tiff')
    for extension in extensions:
        candidate = dataset_dir + '/images/' + sample_name + extension
        if os.path.isfile(candidate):
            image_path = candidate
            break
    for suffix in ('', '_pixels0'):
        for extension in extensions:
            candidate = dataset_dir + '/masks/' + sample_name + suffix + extension
            if os.path.isfile(candidate):
                mask_path = candidate
                break
    ...
    if image_path is None or mask_path is None:
        raise FileNotFoundError(
            'Missing image/mask pair for {} under {}'.format(sample_name, dataset_dir)
        )
    return Image.open(image_path).convert('I'), Image.open(mask_path)
