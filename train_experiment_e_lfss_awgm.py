"""Independent training entry for Experiment E LFSS-before-AWGM variants."""
...
VARIANT_METADATA = {
    "e1_lfss_resblock": {
        "ablation_id": "E1",
        "model_variant": "dwtfreqnet_single_decoder_lfss_awgm_resblock",
        "sd_variant": "sd_awgm_lfss_resblock",
        "output_name": "E1_lfss_resblock",
    },
    "e2_lfss_transition": {
        "ablation_id": "E2",
        "model_variant": "dwtfreqnet_single_decoder_lfss_awgm_transition",
        "sd_variant": "sd_awgm_lfss_transition",
        "output_name": "E2_lfss_transition",
    },
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Experiment E LFSS-preconditioned AWGM"
    )
    parser.add_argument("--dataset-name", required=True)
    parser.add_argument("--dataset-dir", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument(
        "--encoder-variant", required=True, choices=EXPERIMENT_E_VARIANTS
    )
...
def apply_variant_metadata(args):
    metadata = VARIANT_METADATA[args.encoder_variant]
...
def build_model(args, mode):
    return DWTFreqNet_SingleDecoder_LFSS_AWGM(
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Experiment E training")
