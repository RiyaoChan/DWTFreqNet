"""Independent training entry for Experiment G decoder-DSHF ablations."""
...
OUTPUT_NAMES = {
    "g0_e1_passthrough": "G0_e1_passthrough",
    "g1_decoder_dshf": "G1_decoder_dshf",
    "g2_decoder_dshf_semantic": "G2_decoder_dshf_semantic",
    "g3_decoder_dshf_targetness": "G3_decoder_dshf_targetness",
}


def parse_args():
    parser = argparse.ArgumentParser(description="Train Experiment G decoder DSHF")
    ...
    parser.add_argument(
        "--decoder-variant",
        required=True,
        choices=EXPERIMENT_G_VARIANTS[1:],
        help="G0 is a regression/profile reference and is intentionally not trainable here",
    )
...
def apply_variant_metadata(args):
    config = VARIANT_CONFIGS[args.decoder_variant]
    args.ablation_id = config["ablation_id"]
    args.model_variant = f"dwtfreqnet_e1_{args.decoder_variant}"
    args.sd_variant = args.decoder_variant
    args.encoder_variant = "e1_lfss_resblock"


def build_model(args, mode):
    return DWTFreqNet_SingleDecoder_LFSS_AWGM_DecoderDSHF(
...
def evaluate(model, loader, device, threshold):
    ...
    statistics = getattr(model, "last_decoder_dshf_statistics", None)
    if statistics:
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Experiment G training")
    ...
    for stage in range(1, 5):
        ...
        for beta in (refiner.beta_h, refiner.beta_v, refiner.beta_d):
            if not torch.equal(beta, torch.full_like(beta, 1e-3)):
                raise RuntimeError("Experiment G beta initialization is not 1e-3")
    ...
    run_config = {
        ...
        **model.experiment_metadata,
        ...
        "decoder_dshf_parameters": parameter_count(model, ("decoder_hf_refiner",)),
        "semantic_gate_parameters": parameter_count(
            model,
            tuple(
                f"decoder_hf_refiner{stage}.semantic_gate"
                for stage in range(1, 5)
            ),
        ),
        "dwt_calls": 4,
