...
结果更新时间：2026-07-21 15:53（Asia/Shanghai）。NUAA/IRSTD 的最终结果以及 NUDT 的阶段性结果均直接读取服务器对应 `train.log` 的 JSON 评估记录；`best epoch/mIoU` 为截至当前记录的历史最佳，nIoU、F1、Pd、Fa 均取同一 best-mIoU epoch，Fa 越低越好。当前共 13 项完成、5 项运行、0 项排队，无失败或缺失任务；NUDT 的 H1-R 已完成 1000 epoch，其余五项继续训练至 1000 epoch。
...
| H1-D | 898 | 488 | 0.9555 | 0.9546 | 0.9772 | 0.9937 | 1.884e-6 | +0.0039 |
| H2-R | 849 | 471 | 0.9557 | 0.9558 | 0.9774 | 0.9905 | 2.344e-6 | +0.0041 |
| H2-D | 808 | 376 | 0.9570 | 0.9551 | 0.9780 | 0.9937 | 1.609e-6 | +0.0053 |
| **H3-R** | 765 | 513 | **0.9607** | **0.9593** | **0.9799** | **0.9947** | **1.149e-6** | **+0.0091** |
| H3-D | 753 | 488 | 0.9520 | 0.9509 | 0.9754 | 0.9937 | 3.286e-6 | +0.0004 |

截至当前，H3-R 在 epoch 513 取得历史最佳 mIoU/nIoU/F1/Pd/Fa `0.9607/0.9593/0.9799/0.9947/1.149e-6`，mIoU 比 E1 提高 `0.0091`；H1-R 已完成 1000 epoch 且保持 `0.9568` 的最佳 mIoU。H1-R、H1-D、H2-R、H2-D、H3-R 的主指标均超过 E1；由于 H1-D/H2-R/H2-D/H3-R/H3-D 尚未完成 1000 epoch，暂不作最终排序。

### 8.4 当前任务分配

| GPU | 当前任务 | 状态 |
|---:|---|---|
| 0 | Phase 1 P3：NUDT-SIRST test sampling geometry | 运行中 |
| 1 | H2-D / NUDT-SIRST | 训练中 |
| 2 | H1-D / NUDT-SIRST | 训练中 |
| 3 | H2-R / NUDT-SIRST | 训练中 |
| 4 | H3-R / NUDT-SIRST | 训练中 |
| 5 | Phase 1 P3：IRSTD-1K train sampling geometry | 运行中 |
| 6 | H3-D / NUDT-SIRST | 训练中 |

所有未完成的 NUDT 方案仍按要求训练到 1000 epoch，后续最终结果继续更新本节。Phase 1 另在 CPU 上运行 NUDT-SIRST P1 test，并使用 GPU 0、5 执行 P3；这些诊断任务未停止或抢占 H 的 GPU 1/2/3/4/6。服务器复核时每张 H 显卡仅有一个正式 compute 进程，未发现重复 GPU 训练。
