...
| H1-D | **1000** | 488 | 0.9555 | 0.9546 | 0.9772 | 0.9937 | 1.884e-6 | +0.0039 |
| H2-R | **1000** | 471 | 0.9557 | 0.9558 | 0.9774 | 0.9905 | 2.344e-6 | +0.0041 |
| H2-D | **1000** | 376 | 0.9570 | 0.9551 | 0.9780 | 0.9937 | 1.609e-6 | +0.0053 |
| **H3-R** | **1000** | 513 | **0.9607** | **0.9593** | **0.9799** | **0.9947** | **1.149e-6** | **+0.0091** |
| H3-D | **1000** | 488 | 0.9520 | 0.9509 | 0.9754 | 0.9937 | 3.286e-6 | +0.0004 |

NUDT 上 H3-R 在 epoch 513 取得全组最佳 mIoU/nIoU/F1，并以 `0.9947` 的 Pd 和 `1.149e-6` 的 Fa 同时达到本组最优水平；mIoU 比 E1 提高 `0.0091`。H1-R、H1-D、H2-R、H2-D、H3-R 的 mIoU 均超过 E1，H3-D 仅小幅超过 E1，但 Fa 明显更高。

### 8.4 全部实验完成情况

| 数据集 | H1-R | H1-D | H2-R | H2-D | H3-R | H3-D |
|---|---:|---:|---:|---:|---:|---:|
| NUAA-SIRST | 1000 | 1000 | 1000 | 1000 | 1000 | 1000 |
| IRSTD-1K | 1000 | 1000 | 1000 | 1000 | 1000 | 1000 |
| NUDT-SIRST | 1000 | 1000 | 1000 | 1000 | 1000 | 1000 |

Experiment H 已释放全部训练 GPU。三数据集的综合优选分别为：NUAA 选 H2-D（mIoU `0.7963`），IRSTD 选 H2-R（mIoU `0.6726`），NUDT 选 H3-R（mIoU `0.9607`）。其中 H3-R 只在 NUDT 上形成明显优势，低频来源与 Gaussian 形式仍表现出数据集依赖性。
