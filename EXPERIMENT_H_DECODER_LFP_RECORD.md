...
结果更新时间：2026-07-21 09:32（Asia/Shanghai）。指标均直接读取对应任务的 `best_metrics.json`，即按 mIoU 保存的最佳检查点；Fa 越低越好。当前共 12 项完成、6 项运行、0 项排队，无失败或缺失任务。六个 NUDT 任务均已启动并持续训练。
...
六项均已启动，仍在训练至 1000 epoch，因此本节仍是阶段性结果。

| 方案 | 当前 epoch | best epoch | mIoU | nIoU | F1 | Pd | Fa | 相对 E1 mIoU |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| E1/H0 | 1000 | 456 | 0.9516 | 0.9482 | 0.9752 | **0.9947** | 1.724e-6 | — |
| **H1-R** | 897 | 653 | **0.9568** | **0.9570** | **0.9779** | 0.9926 | 2.781e-6 | **+0.0052** |
| H1-D | 370 | 306 | 0.9524 | 0.9509 | 0.9756 | 0.9926 | 2.804e-6 | +0.0008 |
| H2-R | 342 | 329 | 0.9541 | 0.9529 | 0.9765 | 0.9915 | **1.379e-6** | +0.0025 |
| H2-D | 304 | 304 | 0.9540 | 0.9540 | 0.9765 | 0.9915 | 1.884e-6 | +0.0024 |
| H3-R | 279 | 275 | 0.9487 | 0.9483 | 0.9737 | **0.9947** | 4.320e-6 | -0.0029 |
| H3-D | 264 | 223 | 0.9279 | 0.9294 | 0.9626 | 0.9884 | 5.860e-6 | -0.0237 |

H1-R 当前的 mIoU、nIoU 和 F1 最好，三项均高于 E1，但 Pd 略低且 Fa 较高。H2-R 的 mIoU、nIoU、F1 和 Fa 均优于 E1，并取得当前最低 Fa `1.379e-6`，但 Pd 仍略低。由于六项都未训练到 1000 epoch，不作最终排序。

### 8.4 当前任务分配

| GPU | 当前任务 | 状态 |
|---:|---|---|
| 0 | H1-R / NUDT-SIRST | 训练中 |
| 1 | H2-D / NUDT-SIRST | 训练中 |
| 2 | H1-D / NUDT-SIRST | 训练中 |
| 3 | H2-R / NUDT-SIRST | 训练中 |
| 4 | H3-R / NUDT-SIRST | 训练中 |
| 5 | 无 Experiment H 任务 | 空闲 |
| 6 | H3-D / NUDT-SIRST | 训练中 |

所有 NUDT 方案仍按要求训练到 1000 epoch，后续最终结果继续更新本节。
