...
### Stage 1：NUDT-SIRST 400-epoch 初筛

所有任务使用 seed 42、patch size 256、batch size 4、1000-epoch 余弦调度，并在 epoch 400 暂停。优先顺序如下：
...
选取前三个 W8M 方案从 epoch 400 原地恢复到 epoch 1000。

### Stage 2 与 Stage 3

- Stage 2：Stage 1 最好的两个 W8M 版本，加上 `awgm_original`、`dm_awgm_full`、`dm_awgm_no_dcn`，分别在 NUAA-SIRST、NUDT-SIRST、IRSTD-1K 完整训练 1000 epochs。
...
Stage 1 于 2026-07-10 11:49 CST 启动。所有任务使用 NUDT-SIRST、seed 42、batch size 4、patch size 256、1000-epoch 余弦调度，并在 epoch 400 暂停初筛。GPU 队列如下：
