...
### Experiment B：IRSTD-1K 单解码器结果

以下结果来自 226 服务器 `DWTFreqNet_SINGLE_DECODER_B`，两项均已训练到
1000 epoch；指标取验证集最佳 checkpoint。

| 数据集 | 方案 | 最佳 epoch | mIoU | nIoU | F1 | Pd | Fa |
|---|---|---:|---:|---:|---:|---:|---:|
| IRSTD-1K | `sd_raw` | 596 | 65.10 | 64.04 | 78.86 | 90.57 | 20.82 |
| IRSTD-1K | `sd_pyramid` | 632 | 64.77 | 64.46 | 78.62 | 89.90 | 16.21 |
| IRSTD-1K | `sd_awgm` | 894 | **65.61** | **64.77** | **79.24** | **90.91** | **15.37** |
| IRSTD-1K | `sd_full` | 680 | 64.75 | 63.96 | 78.60 | 90.91 | 16.74 |

在这四个单解码器方案中，`sd_awgm` 的 IRSTD-1K mIoU 最高。

### Experiment C：SD-AWGM + Encoder-side LDRC

| 数据集 | 方案 | 状态/当前 epoch | 最佳 epoch | mIoU | nIoU | F1 | Pd | Fa |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| NUAA-SIRST | `sd_awgm_ldrc` | 已完成/1000 | 565 | 77.5521 | 79.1430 | 87.3570 | 96.1832 | 24.0103 |
| NUDT-SIRST | `sd_awgm_ldrc` | 训练中/581 | 382 | 95.6426 | 95.6390 | 97.7728 | 99.5767 | 2.0912 |
| IRSTD-1K | `sd_awgm_ldrc` | 训练中/400 | 382 | 65.0844 | 65.1162 | 78.8499 | 94.6128 | 15.8471 |

以上为 2026-07-14 226 服务器最新阶段性快照；Fa 的单位为 ×10^-6，指标取最佳
checkpoint。NUDT 和 IRSTD-1K 任务仍会继续训练到 1000 epoch。
