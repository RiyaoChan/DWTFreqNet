...
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.instance.lock"
if ! flock -n 9; then
  echo "Another Experiment F queue is already active" >&2
  exit 2
fi
...
output_name() {
  case "$1" in
    f1_multiscale) printf 'F1_multiscale' ;;
    f2_sparse) printf 'F2_sparse' ;;
    f3_cross_direction) printf 'F3_cross_direction' ;;
    f4_low_guided_full) printf 'F4_low_guided_full' ;;
    *) return 2 ;;
  esac
}
...
while true; do
  ...
  if [[ "${active}" -lt "${MAX_CONCURRENT}" ]]; then
    for index in "${!TASK_VARIANTS[@]}"; do
      ...
      nohup bash "${PROJECT_ROOT}/scripts/run_experiment_f_dshf.sh" \
