...
DATASET_TAG=$(printf '%s' "$DATASET" | tr '[:upper:]' '[:lower:]' | tr -c 'a-z0-9' '_')
OUTPUT_ROOT=${OUTPUT_ROOT:-$PROJECT/runs/w8m_stage1_${DATASET_TAG}}
...
if [ "$status" -eq 0 ]; then
  ...
  echo "[$(date --iso-8601=seconds)] finished $DATASET $VARIANT at stage boundary $STOP_EPOCH"
else
