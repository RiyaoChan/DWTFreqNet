#!/usr/bin/env bash
set -euo pipefail

PROJECT_ROOT="${PROJECT_ROOT:-$(cd "$(dirname "$0")/.." && pwd)}"
DATASET_DIR="${DATASET_DIR:-/DATA20T/bip/cry/code/DWTFreqNet_DM_AWGM/datasets}"
PYTHON_BIN="${PYTHON_BIN:-python}"
GPU_LIST="${GPU_LIST:-0,1,2,3,4,5,6}"
POLL_SECONDS="${POLL_SECONDS:-60}"
MAX_USED_MIB="${MAX_USED_MIB:-1024}"
MAX_UTIL_PERCENT="${MAX_UTIL_PERCENT:-5}"
SEED="${SEED:-42}"
RUNTIME_LOG="$PROJECT_ROOT/runs/experiment_d_spatial_ablation/queue_runtime.tsv"

datasets=("NUAA-SIRST" "IRSTD-1K" "NUDT-SIRST")
variants=("d5_same_position" "d6_neighborhood" "d7_target_neighborhood")
IFS=',' read -r -a allowed_gpus <<< "$GPU_LIST"
declare -A scheduler_pid_by_gpu=()
SELECTED_GPU=""

mkdir -p "$(dirname "$RUNTIME_LOG")"
if [[ ! -f "$RUNTIME_LOG" ]]; then
  printf 'timestamp\tid\tdataset\tsd_variant\tseed\tgpu\tpid\toutput_dir\tstatus\n' \
    > "$RUNTIME_LOG"
fi

variant_dir() {
  case "$1" in
    d5_same_position) printf 'D5_same_position' ;;
    d6_neighborhood) printf 'D6_neighborhood' ;;
    d7_target_neighborhood) printf 'D7_target_neighborhood' ;;
    *) return 2 ;;
  esac
}

sd_variant() {
  case "$1" in
    d5_same_position) printf 'sd_awgm_hfe_samepos' ;;
    d6_neighborhood) printf 'sd_awgm_hfe_neighborhood' ;;
    d7_target_neighborhood) printf 'sd_awgm_hfe_targetlocal' ;;
    *) return 2 ;;
  esac
}

ablation_id() {
  case "$1" in
    d5_same_position) printf 'D5' ;;
    d6_neighborhood) printf 'D6' ;;
    d7_target_neighborhood) printf 'D7' ;;
    *) return 2 ;;
  esac
}

output_dir_for() {
  printf 'runs/experiment_d_spatial_ablation/%s/%s/seed%s' \
    "$(variant_dir "$1")" "$2" "$SEED"
}

refresh_running_jobs() {
  local gpu pid
  for gpu in "${!scheduler_pid_by_gpu[@]}"; do
    pid="${scheduler_pid_by_gpu[$gpu]}"
    if ! kill -0 "$pid" 2>/dev/null; then
      wait "$pid" || true
      unset 'scheduler_pid_by_gpu[$gpu]'
    fi
  done
}

gpu_has_compute_pid() {
  local gpu="$1"
  nvidia-smi -i "$gpu" --query-compute-apps=pid --format=csv,noheader,nounits \
    2>/dev/null | grep -q '[0-9]'
}

find_idle_gpu() {
  local gpu used util
  SELECTED_GPU=""
  refresh_running_jobs
  for gpu in "${allowed_gpus[@]}"; do
    [[ -n "${scheduler_pid_by_gpu[$gpu]:-}" ]] && continue
    IFS=',' read -r used util < <(
      nvidia-smi -i "$gpu" --query-gpu=memory.used,utilization.gpu \
        --format=csv,noheader,nounits | tr -d ' '
    )
    if (( used <= MAX_USED_MIB && util <= MAX_UTIL_PERCENT )) && \
      ! gpu_has_compute_pid "$gpu"; then
      SELECTED_GPU="$gpu"
      return 0
    fi
  done
  return 1
}

output_is_active() {
  local output_dir="$1"
  ps -eo args= | grep -F 'train_experiment_d_hfe_spatial_ablation.py' | \
    grep -F -- "--output-dir $output_dir" | grep -v grep >/dev/null
}

launch_task() {
  local variant="$1" dataset="$2"
  local output_dir id sd gpu pid
  output_dir="$(output_dir_for "$variant" "$dataset")"
  id="$(ablation_id "$variant")"
  sd="$(sd_variant "$variant")"
  if output_is_active "$output_dir" || [[ -d "$output_dir/.run.lock" ]]; then
    printf '%s\t%s\t%s\t%s\t%s\t-\t-\t%s\tskipped_active\n' \
      "$(date --iso-8601=seconds)" "$id" "$dataset" "$sd" "$SEED" \
      "$output_dir" | tee -a "$RUNTIME_LOG"
    return
  fi
  if [[ -f "$output_dir/COMPLETED" ]]; then
    printf '%s\t%s\t%s\t%s\t%s\t-\t-\t%s\tskipped_completed\n' \
      "$(date --iso-8601=seconds)" "$id" "$dataset" "$sd" "$SEED" \
      "$output_dir" | tee -a "$RUNTIME_LOG"
    return
  fi
  if [[ -f "$output_dir/FAILED" ]]; then
    printf '%s\t%s\t%s\t%s\t%s\t-\t-\t%s\tskipped_failed\n' \
      "$(date --iso-8601=seconds)" "$id" "$dataset" "$sd" "$SEED" \
      "$output_dir" | tee -a "$RUNTIME_LOG"
    return
  fi
  until find_idle_gpu; do
    sleep "$POLL_SECONDS"
  done
  gpu="$SELECTED_GPU"
  mkdir -p "$output_dir"
  PATH="$(dirname "$PYTHON_BIN"):$PATH" PYTHON_BIN="$PYTHON_BIN" \
    bash scripts/run_experiment_d_hfe_spatial_ablation.sh \
      "$variant" "$dataset" "$gpu" "$SEED" "$DATASET_DIR" \
      > "$output_dir/launcher.log" 2>&1 &
  pid=$!
  scheduler_pid_by_gpu[$gpu]="$pid"
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\trunning\n' \
    "$(date --iso-8601=seconds)" "$id" "$dataset" "$sd" "$SEED" \
    "$gpu" "$pid" "$output_dir" | tee -a "$RUNTIME_LOG"
}

cd "$PROJECT_ROOT"
for variant in "${variants[@]}"; do
  for dataset in "${datasets[@]}"; do
    launch_task "$variant" "$dataset"
  done
done
for gpu in "${!scheduler_pid_by_gpu[@]}"; do
  wait "${scheduler_pid_by_gpu[$gpu]}" || true
done
