...
if ! flock -n 9; then echo "Another Experiment G queue is active" >&2; exit 2; fi
...
TASK_VARIANTS=(
  g1_decoder_dshf g2_decoder_dshf_semantic g3_decoder_dshf_targetness
  g1_decoder_dshf g2_decoder_dshf_semantic g3_decoder_dshf_targetness
  g1_decoder_dshf g2_decoder_dshf_semantic g3_decoder_dshf_targetness
)
TASK_DATASETS=(
  ...
  NUDT-SIRST NUDT-SIRST NUDT-SIRST
)

output_name() {
  case "$1" in
    g1_decoder_dshf) printf 'G1_decoder_dshf' ;;
    g2_decoder_dshf_semantic) printf 'G2_decoder_dshf_semantic' ;;
    g3_decoder_dshf_targetness) printf 'G3_decoder_dshf_targetness' ;;
  esac
}
...
while true; do
  ...
  if [[ "${active}" -lt "${MAX_CONCURRENT}" ]]; then
    for index in "${!TASK_VARIANTS[@]}"; do
      variant="${TASK_VARIANTS[index]}"; dataset="${TASK_DATASETS[index]}"
      ...
      PROJECT_ROOT="${PROJECT_ROOT}" DATASET_DIR="${DATASET_DIR}" PYTHON_BIN="${PYTHON_BIN}" OUTPUT_ROOT="${OUTPUT_ROOT}" \
        nohup bash "${PROJECT_ROOT}/scripts/run_experiment_g_decoder_dshf.sh" "${variant}" "${dataset}" "${selected_gpu}" "${SEED}" >/dev/null 2>&1 9>&- &
