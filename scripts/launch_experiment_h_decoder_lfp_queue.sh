...
if ! flock -n 9; then echo "Another Experiment H queue is active" >&2; exit 2; fi
...
ONE_SET=(h1_rawll_attention h1_decoder_attention h2_rawll_fixed_gaussian h2_decoder_fixed_gaussian h3_rawll_adaptive_gaussian h3_decoder_adaptive_gaussian)
TASK_VARIANTS=("${ONE_SET[@]}" "${ONE_SET[@]}" "${ONE_SET[@]}")
TASK_DATASETS=(
  NUAA-SIRST NUAA-SIRST NUAA-SIRST NUAA-SIRST NUAA-SIRST NUAA-SIRST
  IRSTD-1K IRSTD-1K IRSTD-1K IRSTD-1K IRSTD-1K IRSTD-1K
  NUDT-SIRST NUDT-SIRST NUDT-SIRST NUDT-SIRST NUDT-SIRST NUDT-SIRST
)

output_name() {
  case "$1" in
    h1_rawll_attention) printf H1_rawll_attention ;;
    h1_decoder_attention) printf H1_decoder_attention ;;
    h2_rawll_fixed_gaussian) printf H2_rawll_fixed_gaussian ;;
    h2_decoder_fixed_gaussian) printf H2_decoder_fixed_gaussian ;;
    h3_rawll_adaptive_gaussian) printf H3_rawll_adaptive_gaussian ;;
    h3_decoder_adaptive_gaussian) printf H3_decoder_adaptive_gaussian ;;
  esac
}
...
while true; do
  if all_complete; then printf '%s\tqueue_complete\n' "$(date --iso-8601=seconds)" >> "${QUEUE_LOG}"; exit 0; fi
  active="$(active_count)"
  if [[ "${active}" -lt "${MAX_CONCURRENT}" ]]; then
    for index in "${!TASK_VARIANTS[@]}"; do
      variant="${TASK_VARIANTS[index]}"; dataset="${TASK_DATASETS[index]}"; dir="$(task_dir "${variant}" "${dataset}")"
      [[ "$(task_state "${dir}")" == queued ]] || continue
      selected_gpu=""
      for gpu in "${GPUS[@]}"; do if gpu_is_free "${gpu}"; then selected_gpu="${gpu}"; break; fi; done
      [[ -n "${selected_gpu}" ]] || break
      PROJECT_ROOT="${PROJECT_ROOT}" DATASET_DIR="${DATASET_DIR}" PYTHON_BIN="${PYTHON_BIN}" OUTPUT_ROOT="${OUTPUT_ROOT}" \
        nohup bash "${PROJECT_ROOT}/scripts/run_experiment_h_decoder_lfp.sh" "${variant}" "${dataset}" "${selected_gpu}" "${SEED}" >/dev/null 2>&1 9>&- &
      wrapper_pid=$!; timestamp="$(date --iso-8601=seconds)"
      printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\trunning\n' "${timestamp}" "${variant}" "${dataset}" "${SEED}" "${selected_gpu}" "${wrapper_pid}" "${dir}" | tee -a "${RUNTIME_TSV}" >> "${QUEUE_LOG}"
      active=$((active + 1)); [[ "${active}" -lt "${MAX_CONCURRENT}" ]] || break; sleep 2
    done
  fi
  sleep "${POLL_SECONDS}"
done
