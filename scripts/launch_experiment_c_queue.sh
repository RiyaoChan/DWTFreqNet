...
SEED="${SEED:-42}"
RUNTIME_LOG="$PROJECT_ROOT/runs/experiment_c/queue_runtime.tsv"
...
if [[ ! -f "$RUNTIME_LOG" ]]; then
  printf 'timestamp\tid\tdataset\tvariant\tseed\tgpu\tpid\toutput_dir\tstatus\n' > "$RUNTIME_LOG"
fi
...
output_is_active() {
  local output_dir="$1"
  ps -eo args= | grep -F "train_experiment_c.py" | \
