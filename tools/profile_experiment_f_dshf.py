"""Complexity, latency and memory profile for Experiment F0/F1/F4/F2/F3."""
...
NAMES = ("f0_e1", *EXPERIMENT_F_LAUNCH_ORDER)
...
def profile_one(name, device, warmup, repeats):
    ...
    dshf_parameters = (
        count_prefix(model, ("dir_encoder",)) if name != "f0_e1" else 0
    )
    direction_parameters = count_prefix(model, ("dir_encoder",))
