"""Parameters, FLOPs, latency, memory and component runtimes for Experiment K."""
...
PROFILE_VARIANTS = EXPERIMENT_K_VARIANTS[:6]
...
def component_timings(model, device, warmup, repeats, model_latency):
    names = ("mad", "gaussian", "gaussian_radial", "k_complete")
    if not model.use_k_purification:
        return {name: {"latency_ms": 0.0, "model_latency_fraction": 0.0} for name in names}
    bands, priors = {}, {}
    prior_channels = {1: 32, 2: 64, 3: 128, 4: 256}
    for stage, channels in K_STAGE_CHANNELS.items():
        spatial = 256 // (2 ** stage)
        bands[stage] = tuple(
            torch.randn(1, channels, spatial, spatial, device=device) for _ in range(3)
        )
        priors[stage] = torch.randn(
            1, prior_channels[stage], spatial, spatial, device=device
        )
    calls = {
        "mad": lambda: [
            getattr(model, f"decoder_k{stage}").noise_estimators[band].robust_scale(
                bands[stage][index]
            )
            for stage in range(1, 5) for index, band in enumerate("HVD")
        ],
        "gaussian": lambda: [
            getattr(model, f"decoder_k{stage}").gaussians[band](bands[stage][index])
            for stage in range(1, 5) for index, band in enumerate("HVD")
        ],
        "k_complete": lambda: [
            getattr(model, f"decoder_k{stage}")(
                *bands[stage],
                prior_low=priors[stage] if model.k_source_map[stage] else None,
            )
            for stage in range(1, 5)
        ],
    }
    compactness_calls = [
        (getattr(model, f"decoder_k{stage}").compactness, priors[stage])
        for stage in range(1, 5) if model.k_source_map[stage]
    ]
    calls["gaussian_radial"] = lambda: [
        module(tensor) for module, tensor in compactness_calls
    ]
    results = {}
    for name in names:
        if name == "gaussian_radial" and not compactness_calls:
            latency, deviation = 0.0, 0.0
        else:
            latency, deviation = timed(calls[name], device, warmup, repeats)
        results[name] = {
            "latency_ms": latency,
...
def profile_one(variant, device, warmup, repeats):
    ...
    groups = {
        ...
        "k_total": count_prefix(model, ("decoder_k",)),
        "k_alpha": count_prefix(model, tuple(
            f"decoder_k{stage}.alpha_logits" for stage in range(1, 5)
        )),
        "k_rho": count_prefix(model, tuple(
            f"decoder_k{stage}.prior_protection.rho_logit" for stage in range(1, 5)
        )),
        "k_noise": count_prefix(model, tuple(
            f"decoder_k{stage}.noise_estimators" for stage in range(1, 5)
        )),
        "k_gaussian": count_prefix(model, tuple(
            f"decoder_k{stage}.gaussians" for stage in range(1, 5)
        )),
    }
...
def main():
    ...
    if args.repeats < 20:
        raise ValueError("Experiment K requires at least 20 profile repeats")
    device = torch.device(args.device)
    results = [profile_one(variant, device, args.warmup, args.repeats) for variant in PROFILE_VARIANTS]
    ...
    payload = {
        ...
        "thop_note": (
            "3x3 depthwise Gaussian MACs use an explicit handler; median, grid_sample, "
            "comparisons and selective_scan may be omitted, so FLOPs are relative estimates."
        ),
        "component_note": (
            "MAD, Gaussian and Gaussian-radial times are isolated four-stage microbenchmarks, "
            "not additive end-to-end attribution."
        ),
        "results": results,
    }
