"""Independent 1000-epoch training entry for Experiment K v2."""
...
FORMAL_VARIANTS = EXPERIMENT_K_VARIANTS[2:]
...
def parse_args():
    ...
    parser.add_argument("--k-variant", required=True, choices=FORMAL_VARIANTS)
...
def apply_variant_metadata(args):
    args.ablation_id = K_VARIANT_CONFIGS[args.k_variant]["id"]
    args.model_variant = f"dwtfreqnet_e1_{args.k_variant}"
    args.sd_variant = args.k_variant
    args.encoder_variant = "e1_lfss_resblock"


def build_model(args, mode):
    return DWTFreqNet_SingleDecoder_LFSS_AWGM_K(
        get_DWTFreqNet_config(), k_variant=args.k_variant,
        decision_path=args.decision_json, mode=mode, deepsuper=True,
    )
...
def validate_checkpoint(checkpoint, args, path):
    ...
    if actual != expected:
        raise RuntimeError(
            f"Checkpoint architecture mismatch for {path}: checkpoint={actual}, requested={expected}"
        )


def evaluate(model, loader, device, threshold):
    result = evaluate_base(model, loader, device, threshold)
    statistics = getattr(model, "last_k_statistics", None)
    if statistics:
...
def validate_initialization(model):
    ...
    for stage in range(1, 5):
        ...
        if not torch.allclose(
            processor.alpha.detach(), torch.full_like(processor.alpha.detach(), 0.05), atol=1e-6
        ):
            raise RuntimeError("Experiment K alpha initialization is not 0.05")
        alpha_count += int(processor.alpha_logits.numel())
        for band in ("H", "V", "D"):
            if abs(float(processor.noise_estimators[band].lambda_value.detach()) - 1.0) > 1e-6:
                raise RuntimeError("Experiment K lambda initialization is not 1.0")
            if abs(float(processor.gaussians[band].sigma.detach()) - 1.0) > 1e-6:
                raise RuntimeError("Experiment K Gaussian sigma initialization is not 1.0")
        if processor.protection_enabled:
            if abs(float(processor.rho.detach()) - 0.05) > 1e-6:
                raise RuntimeError("Experiment K rho initialization is not 0.05")
            rho_count += 1
    if alpha_count != 12:
        raise RuntimeError(f"Experiment K requires 12 alpha parameters, got {alpha_count}")
    expected_rho = sum(source is not None for source in model.k_source_map.values())
    if rho_count != expected_rho:
        raise RuntimeError(f"Expected {expected_rho} rho parameters, got {rho_count}")
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Experiment K training")
    set_seed(args.seed)
    ...
    train_set = TrainSetLoader(
        args.dataset_dir, args.dataset_name, patch_size=args.patch_size, img_norm_cfg=None
    )
    ...
    initialization_max_difference = lfss_initialization_max_difference(lfss_before, lfss_after)
    if initialization_max_difference != 0.0:
        raise RuntimeError(
            "Initialization modified LFSS protected parameters: "
    ...
    run_config = {
        ...
        **model.experiment_metadata,
        ...
        "k_parameters": parameter_count(model, ("decoder_k",)),
        "k_alpha_parameters": parameter_count(
            model, tuple(f"decoder_k{stage}.alpha_logits" for stage in range(1, 5))
        ),
        "k_rho_parameters": parameter_count(
            model, tuple(f"decoder_k{stage}.prior_protection.rho_logit" for stage in range(1, 5))
        ),
        "dwt_calls": 4,
        "idwt_calls": 4,
    ...
    optimizer, scheduler = get_optimizer(
        model, "Adam", "CosineAnnealingLR", {"lr": args.lr},
    ...
    end_epoch = min(args.epochs, args.stop_after_epoch) if args.stop_after_epoch > 0 else args.epochs
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        should_evaluate = not args.skip_evaluation and (
            (epoch >= args.eval_start and epoch % args.eval_every == 0) or epoch == end_epoch
        )
        if should_evaluate:
            ...
            if result["mIoU"] > best_miou:
                ...
                save_checkpoint(
                    output_dir / "best.pth.tar", epoch, model, optimizer,
                    scheduler, best_miou, args,
                )
        ...
        if epoch % args.save_every == 0 or epoch == end_epoch:
            save_checkpoint(
                output_dir / "latest.pth.tar", epoch, model, optimizer,
                scheduler, best_miou, args,
            )
        writer.flush()
    writer.close()
