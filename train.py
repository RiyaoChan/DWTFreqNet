...
parser.add_argument("--optimizer_name", default='Adam', type=str, help="optimizer name: AdamW, Adam, Adagrad, SGD")#SCT是Adam
parser.add_argument("--epochs", default=1000, type=int, help="optimizer name: AdamW, Adam, Adagrad, SGD")
parser.add_argument("--begin_test", default=100, type=int)#100
parser.add_argument("--every_test", default=5, type=int)#5
parser.add_argument("--every_save_pth", default=20, type=int)
parser.add_argument("--every_print", default=5, type=int)#5
...
parser.add_argument("--log_dir", type=str, default="./otherlogs/DWTFreqNet", help='path of log files')
...
def train():
    ...
    if opt.resume:
        ...
        ckpt = torch.load('/home/cvpr/mqw/about_IR/DWT.pth.tar')
    ...
    ### Default settings of DWTFreqNet
    if opt.optimizer_name == 'Adam':
        opt.optimizer_settings = {'lr': 0.001}###0.001
    ...
    if opt.optimizer_name == 'Adagrad':
        opt.optimizer_settings = {'lr': 0.01}##之前0.05
        opt.scheduler_name = 'CosineAnnealingLR'
        opt.scheduler_settings = {'epochs': opt.epochs, 'eta_min': 1e-5, 'min_lr': 1e-5}
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        if idx_epoch == 0:##before 0, if resume
            best_mIOU = results1
            best_Pd = results2
