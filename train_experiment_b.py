"""Independent training entry for Experiment B single-decoder variants."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Experiment B on one dataset")
    parser.add_argument("--dataset-name", required=True)
    parser.add_argument("--dataset-dir", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--sd-variant", required=True, choices=SINGLE_DECODER_VARIANTS)
...
def validate_checkpoint(checkpoint, args, path):
    ...
    if checkpoint_model != MODEL_VARIANT or checkpoint_variant != args.sd_variant:
        raise RuntimeError(
            ...
            f"requested model_variant={MODEL_VARIANT!r}, sd_variant={args.sd_variant!r}."
        )
...
def main():
    ...
    pyramid_parameters = parameter_count(
        model, ("directional_pyramid", "delta_", "beta_")
    )
    run_config = {
