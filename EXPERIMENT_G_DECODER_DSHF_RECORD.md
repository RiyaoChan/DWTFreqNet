...
结果更新时间：2026-07-19 17:15（Asia/Shanghai）。以下指标均直接读取各正式任务的 `best_metrics.json`；Fa 越低越好。
...
### 5.3 NUDT-SIRST 阶段性结果

NUDT-SIRST 尚未完成，因此本节不能作为最终结论。更新时间点的进度为 G1 epoch 789、G2 epoch 641、G3 epoch 633。

| 方案 | 当前 epoch | best epoch | mIoU | nIoU | F1 | Pd | Fa | 相对 G0 mIoU |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| G0 / E1 | 1000 | 456 | **0.9516** | 0.9482 | **0.9752** | 0.9947 | **1.724e-6** | — |
| **G1** | 789 | 432 | 0.9509 | **0.9519** | 0.9748 | 0.9915 | **2.918e-6** | -0.0007 |
| G2 | 641 | 398 | 0.9502 | 0.9508 | 0.9745 | **0.9968** | 3.171e-6 | -0.0014 |
| G3 | 633 | 513 | 0.9496 | 0.9505 | 0.9742 | 0.9915 | 4.090e-6 | -0.0020 |

当前三个 G 方案的 nIoU 均超过 E1；G1 的 mIoU 距离 E1 仅 0.0007，暂时是 NUDT-SIRST 最优 G 方案，但三项必须训练到 1000 epoch 后再形成最终结论。

### 5.4 当前运行状态

- 已完成：NUAA-SIRST × G1/G2/G3，IRSTD-1K × G1/G2/G3，共 6 项。
- 运行中：NUDT-SIRST × G1/G2/G3，共 3 项，分别使用 GPU 5、0、2。
- 动态队列正常，无 `FAILED` 标记；GPU 1、3、4、6 已空闲。
- 当前推荐：NUAA-SIRST 使用 G3，IRSTD-1K 使用 G1；NUDT-SIRST 暂定 G1，等待最终结果。
