...
| P3（GPU，6项） | 5项完成、1项运行 | 空闲GPU调度器PID `3598996` | NUAA train/test、NUDT train/test、IRSTD train完成；IRSTD test运行中 |
...
`scripts/monitor_phase1_idle_gpus.sh` 于 `2026-07-21 11:59:33` 启动，每30秒检查全部GPU。只有同时满足显存低于1000 MiB、利用率低于10%且无compute PID时才认领任务；监控PID `3598996` 截至23:14仍正常存活。调度器已经补齐P2、P3 train和三项H交叉分析；NUDT P3 test 已于22:31发布正式summary，当前仅GPU 5上的IRSTD P3 test尚未完成，监控器继续负责防重复发布与最终收尾。
...
结果更新时间：2026-07-21 23:14（Asia/Shanghai）。本节只记录正式全量输出；P1、P2和H交叉分析均已完成，P3仅剩IRSTD测试集任务。最终跨数据集结论仍等待该P3测试和最终聚合。
...
| IRSTD-1K | test | 201 | 固定使用train选择 | 计算中 | 运行中（GPU 5，140/201） |

NUAA已完成发现/确认隔离并为Go；NUDT train/test均选择所有stage的`Rmax=2`并通过公平性检查，正式判定为Go。IRSTD train同样选择所有stage的`Rmax=2`并通过公平性检查，test仍在以该半径执行完整中心扰动验证。
...
- 可正式记录：P1 NUAA/IRSTD为Go、NUDT为Partial Go；P2全部结论；P3 NUAA和NUDT为Go；三数据集的H交叉统计已完成。
- 仅阶段性记录：P3 IRSTD train为Go，IRSTD test仍在运行。
- 尚不可定论：P3 IRSTD test与最终聚合。
