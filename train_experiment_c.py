"""Independent training entry for Experiment C SD-AWGM + encoder LDRC."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Experiment C on one dataset")
...
def validate_checkpoint(checkpoint, path):
    ...
    if checkpoint_model != MODEL_VARIANT or checkpoint_variant != EXPERIMENT_C_VARIANT:
        raise RuntimeError(
            ...
            f"requested model_variant={MODEL_VARIANT!r}, "
            f"sd_variant={EXPERIMENT_C_VARIANT!r}."
        )
...
def main():
    ...
    args.sd_variant = EXPERIMENT_C_VARIANT
    ...
    ldrc_parameters = parameter_count(model, ("encoder_ldrc",))
    ...
    print(json.dumps({
        ...
        "sd_variant": EXPERIMENT_C_VARIANT,
