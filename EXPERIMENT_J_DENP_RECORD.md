...
|---|---|---|---|---|---|
| NUAA-SIRST | GPU 1运行，epoch 273 | GPU 2运行，epoch 272 | GPU 3运行，epoch 267 | GPU 4运行，epoch 201 | GPU 6首次启动失败并保留；未自动重跑 |
| IRSTD-1K | GPU 6运行，epoch 57，尚未开始评估 | 待启动 | 待启动 | 待启动 | 待启动 |
| NUDT-SIRST | 待启动 | 待启动 | 待启动 | 待启动 | 待启动 |

状态更新时间：`2026-07-21 19:34`（Asia/Shanghai）。队列PID `3889915` 正常存活；当前5项运行、1项失败保留、9项排队、0项完成。运行时权重不提交GitHub。
...
- 重跑：监控器未自动重跑该正式输出，等待明确授权后再以保留失败证据的方式安排。
...
下列 E1/J0 是既有基线，不是本轮重复训练结果。J1–J3 的数值为截至 `2026-07-21 19:34` 从正式 `best_metrics.json` 读取的阶段性best；所有运行项尚未完成1000 epoch，不能作为最终排名。

| Dataset | Variant | Best epoch | mIoU | nIoU | F1 | Pd | Fa |
|---|---|---:|---:|---:|---:|---:|---:|
| NUAA | E1/J0 | 595 | 0.7842 | 0.7960 | 0.8790 | 0.9656 | 2.717e-5 |
| NUAA | J1 | 266（阶段） | 0.7630 | 0.7758 | 0.8656 | 0.9580 | 3.341e-5 |
| NUAA | J2-R | 223（阶段） | 0.7635 | 0.7845 | 0.8659 | 0.9618 | 3.574e-5 |
| NUAA | **J2-D** | 227（阶段） | **0.7678** | **0.7853** | **0.8686** | 0.9580 | 3.396e-5 |
| NUAA | J3-F | 163（阶段） | 0.7489 | 0.7731 | 0.8564 | 0.9580 | **2.758e-5** |
| NUAA | J3-R | 首次启动失败 | — | — | — | — | — |
...
当前只有NUAA四个变体产生阶段性评估，尚不足以给出跨数据集Go/No-Go。阶段上J2-D的mIoU最高（`0.7678`），但仍低于E1/J0的`0.7842`；J3-F的Fa最低（`2.758e-5`），与E1/J0的`2.717e-5`接近但尚未更低。J3-R首次启动失败来自初始化校验的CPU/CUDA设备不一致，修正已提交，失败证据保留且未自动覆盖或重跑。其余结论继续标记为“待实验”，不是No-Go。
