...
> 状态：代码、服务器 CUDA/真实数据验证和 RTX 3090 复杂度测试均已完成；15 项正式训练队列持续运行。截至2026-07-22 22:03，NUAA与IRSTD各五项、NUDT-J1均已完成1000 epoch，NUDT其余四项正在运行，当前无排队任务。J3-R首次失败输出已独立归档，修正设备校验后的自动重跑已完成。所有表格数值直接来自对应 `best_metrics.json`，严禁用占位值或当前epoch指标冒充best。
...
| NUDT-SIRST | **完成1000，best 646** | 运行：epoch 778，best 642 | 运行：epoch 537，best 523 | 运行：epoch 239，best 234 | 运行：epoch 227，best 226 |

状态更新时间：`2026-07-22 22:03`（Asia/Shanghai）。队列PID `3889915` 正常存活；当前4项运行、11项完成、0项排队，另保留J3-R–NUAA的首次失败归档。NUAA与IRSTD各五个变体均已完成1000 epoch，其中NUAA-J3-R自动重跑也已正常完成；NUDT-J1完成，J2-R、J2-D、J3-F和J3-R分别在GPU 0、6、1、2上运行。运行时权重不提交GitHub。
...
下列 E1/J0 是既有基线，不是本轮重复训练结果。表中所有数值均为截至 `2026-07-22 22:03` 从各正式运行目录读取：当前epoch来自 `metrics.jsonl` 的最后一条记录，best epoch及五项指标来自 `best_metrics.json`。运行中任务仅代表当前阶段best，不作为最终结果；未产生best文件的任务明确标记为“待训练”，不会用当前epoch指标冒充best。
...
| NUDT | J2-R | 运行 | 778 | 642 | 0.9545 | 0.9556 | 0.9767 | 0.9915 | 1.310e-6 |
| NUDT | **J2-D** | 运行 | 537 | **523** | **0.9561** | **0.9563** | **0.9775** | 0.9915 | **9.881e-7** |
| NUDT | J3-F | 运行 | 239 | 234 | 0.9390 | 0.9434 | 0.9685 | 0.9905 | 6.228e-6 |
| NUDT | J3-R | 运行 | 227 | 226 | 0.9417 | 0.9410 | 0.9700 | **0.9937** | 4.918e-6 |
...
NUAA五项均已完成1000 epoch：J1最终best为 mIoU/nIoU/F1/Pd/Fa `0.7962/0.8004/0.8865/0.9733/1.955e-5`，相对E1/J0的mIoU提升 `0.0119`；J2-R取得最低Fa `1.825e-5`；J3-F取得最高Pd `0.9771`；J3-R自动重跑完成但mIoU仅为 `0.7748`，未优于J3-F或E1。IRSTD五项也已全部完成：J1取得最高mIoU/F1 `0.6711/0.8032`，J2-R取得最高nIoU `0.6687`，J2-D取得最低Fa `8.445e-6`，J3-F取得最高Pd `0.9360`；这些优势分别高于E1/J0对应的 `0.6647/0.7985`、`0.6630`、`1.230e-5`和`0.9158`。NUDT-J1已经完成，最终best为 `0.9506/0.9504/0.9747/0.9926/1.287e-6`；四个后续变体仍在运行，其中J2-D阶段best在epoch 523达到 mIoU/nIoU/F1/Fa `0.9561/0.9563/0.9775/9.881e-7`，均优于E1/J0的 `0.9516/0.9482/0.9752/1.724e-6`；J3-R当前阶段最高Pd为 `0.9937`，仍略低于E1/J0的 `0.9947`。NUDT四项尚未完成，因此跨数据集Go/No-Go仍需等待最终结果与正式诊断。
