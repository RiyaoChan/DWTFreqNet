...
> 状态：代码、服务器 CUDA/真实数据验证和 RTX 3090 复杂度测试均已完成；15 项正式训练队列持续运行。截至2026-07-22 10:49，NUAA五项均已完成1000 epoch，IRSTD五项和NUDT两项正在运行，NUDT其余三项排队。J3-R首次失败输出已独立归档，修正设备校验后的自动重跑已完成。所有表格数值直接来自对应 `best_metrics.json`，严禁用占位值或当前epoch指标冒充best。
...
| IRSTD-1K | 运行：epoch 933，best 441 | 运行：epoch 590，best 551 | 运行：epoch 586，best 574 | 运行：epoch 562，best 539 | 运行：epoch 511，best 487 |
| NUDT-SIRST | 运行：epoch 447，best 353 | 运行：epoch 237，best 217 | 待启动 | 待启动 | 待启动 |

状态更新时间：`2026-07-22 10:49`（Asia/Shanghai）。队列PID `3889915` 正常存活；当前7项运行、5项完成、3项排队，另保留J3-R–NUAA的首次失败归档。NUAA五个变体均已完成1000 epoch，其中J3-R自动重跑也已正常完成；IRSTD五项全部运行，NUDT的J1/J2-R正在运行，其余三项等待空闲GPU。运行时权重不提交GitHub。
...
下列 E1/J0 是既有基线，不是本轮重复训练结果。表中所有数值均为截至 `2026-07-22 10:49` 从各正式运行目录读取：当前epoch来自 `metrics.jsonl` 的最后一条记录，best epoch及五项指标来自 `best_metrics.json`。运行中任务仅代表当前阶段best，不作为最终结果；未产生best文件的任务明确标记为“待训练”，不会用当前epoch指标冒充best。
...
| IRSTD | **J1** | 运行 | 933 | **441** | **0.6711** | 0.6372 | **0.8032** | 0.9125 | 2.970e-5 |
| IRSTD | J2-R | 运行 | 590 | 551 | 0.6649 | **0.6686** | 0.7987 | **0.9293** | 1.342e-5 |
| IRSTD | J2-D | 运行 | 586 | 574 | 0.6600 | 0.6546 | 0.7952 | 0.9091 | **1.032e-5** |
| IRSTD | J3-F | 运行 | 562 | 539 | 0.6558 | 0.6524 | 0.7922 | 0.9259 | 1.934e-5 |
| IRSTD | J3-R | 运行 | 511 | 487 | 0.6537 | 0.6570 | 0.7906 | 0.9091 | 1.509e-5 |
| NUDT | E1/J0 | 基线 | 1000 | 456 | 0.9516 | 0.9482 | 0.9752 | 0.9947 | 1.724e-6 |
| NUDT | **J1** | 运行 | 447 | **353** | **0.9499** | **0.9508** | **0.9743** | **0.9937** | 3.769e-6 |
| NUDT | J2-R | 运行 | 237 | 217 | 0.9452 | 0.9427 | 0.9719 | 0.9926 | **8.732e-7** |
| NUDT | J2-D | 待训练 | — | — | — | — | — | — | — |
| NUDT | J3-F | 待训练 | — | — | — | — | — | — | — |
| NUDT | J3-R | 待训练 | — | — | — | — | — | — | — |
...
NUAA五项均已完成1000 epoch：J1最终best为 mIoU/nIoU/F1/Pd/Fa `0.7962/0.8004/0.8865/0.9733/1.955e-5`，相对E1/J0的mIoU提升 `0.0119`；J2-R取得最低Fa `1.825e-5`；J3-F取得最高Pd `0.9771`；J3-R自动重跑完成但mIoU仅为 `0.7748`，未优于J3-F或E1。IRSTD当前J1阶段best mIoU为 `0.6711`，已高于E1/J0的 `0.6647`，但其nIoU与Fa仍不占优；J2-R取得当前最高nIoU `0.6686`，J2-D取得当前最低Fa `1.032e-5`。NUDT当前J1阶段best mIoU为 `0.9499`，仍低于E1/J0的 `0.9516`；J2-R当前Fa `8.732e-7` 优于E1/J0，但mIoU明显较低。IRSTD和NUDT任务尚未全部完成，因此跨数据集Go/No-Go仍需等待最终结果。
