...
> 状态：代码、服务器 CUDA/真实数据验证和 RTX 3090 复杂度测试均已完成；15 项正式训练队列持续运行。截至2026-07-22 16:49，NUAA五项和IRSTD-J1均已完成1000 epoch，IRSTD其余四项与NUDT三项正在运行，NUDT其余两项排队。J3-R首次失败输出已独立归档，修正设备校验后的自动重跑已完成。所有表格数值直接来自对应 `best_metrics.json`，严禁用占位值或当前epoch指标冒充best。
...
| IRSTD-1K | **完成1000，best 441** | 运行：epoch 892，best 668 | 运行：epoch 889，best 635 | 运行：epoch 855，best 613 | 运行：epoch 796，best 794 |
| NUDT-SIRST | 运行：epoch 716，best 646 | 运行：epoch 494，best 472 | 运行：epoch 254，best 238 | 待启动 | 待启动 |

状态更新时间：`2026-07-22 16:49`（Asia/Shanghai）。队列PID `3889915` 正常存活；当前7项运行、6项完成、2项排队，另保留J3-R–NUAA的首次失败归档。NUAA五个变体均已完成1000 epoch，其中J3-R自动重跑也已正常完成；IRSTD-J1完成，其余四项运行；NUDT的J1/J2-R/J2-D正在运行，J3-F/J3-R等待空闲GPU。运行时权重不提交GitHub。
...
下列 E1/J0 是既有基线，不是本轮重复训练结果。表中所有数值均为截至 `2026-07-22 16:49` 从各正式运行目录读取：当前epoch来自 `metrics.jsonl` 的最后一条记录，best epoch及五项指标来自 `best_metrics.json`。运行中任务仅代表当前阶段best，不作为最终结果；未产生best文件的任务明确标记为“待训练”，不会用当前epoch指标冒充best。
...
| IRSTD | J2-R | 运行 | 892 | 668 | 0.6685 | **0.6687** | 0.8013 | 0.9226 | 1.120e-5 |
| IRSTD | J2-D | 运行 | 889 | 635 | 0.6666 | 0.6605 | 0.8000 | 0.9091 | **8.445e-6** |
| IRSTD | J3-F | 运行 | 855 | 613 | 0.6620 | 0.6588 | 0.7966 | **0.9360** | 1.236e-5 |
| IRSTD | J3-R | 运行 | 796 | 794 | 0.6573 | 0.6523 | 0.7932 | 0.9057 | 1.135e-5 |
| NUDT | E1/J0 | 基线 | 1000 | 456 | 0.9516 | 0.9482 | 0.9752 | 0.9947 | 1.724e-6 |
| NUDT | J1 | 运行 | 716 | 646 | 0.9506 | 0.9504 | 0.9747 | **0.9926** | 1.287e-6 |
| NUDT | **J2-R** | 运行 | 494 | **472** | **0.9530** | **0.9546** | **0.9759** | 0.9915 | **1.241e-6** |
| NUDT | J2-D | 运行 | 254 | 238 | 0.9383 | 0.9379 | 0.9682 | **0.9926** | 3.470e-6 |
| NUDT | J3-F | 待训练 | — | — | — | — | — | — | — |
| NUDT | J3-R | 待训练 | — | — | — | — | — | — | — |
...
NUAA五项均已完成1000 epoch：J1最终best为 mIoU/nIoU/F1/Pd/Fa `0.7962/0.8004/0.8865/0.9733/1.955e-5`，相对E1/J0的mIoU提升 `0.0119`；J2-R取得最低Fa `1.825e-5`；J3-F取得最高Pd `0.9771`；J3-R自动重跑完成但mIoU仅为 `0.7748`，未优于J3-F或E1。IRSTD-J1也已完成1000 epoch，最终best mIoU为 `0.6711`，高于E1/J0的 `0.6647`，但nIoU和Fa不占优；当前J2-R的nIoU达到 `0.6687`，J2-D的Fa降至 `8.445e-6`，J3-F的Pd达到 `0.9360`。NUDT当前J2-R阶段best在epoch 472更新为 mIoU/nIoU/F1/Fa `0.9530/0.9546/0.9759/1.241e-6`，四项均优于E1/J0的 `0.9516/0.9482/0.9752/1.724e-6`；J1与J2-D当前最高Pd均为 `0.9926`，仍低于E1/J0的 `0.9947`。IRSTD和NUDT任务尚未全部完成，因此跨数据集Go/No-Go仍需等待最终结果。
