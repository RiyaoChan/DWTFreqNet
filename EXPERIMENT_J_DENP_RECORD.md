...
> 状态：代码、服务器 CUDA/真实数据验证和 RTX 3090 复杂度测试均已完成；15 项正式训练队列持续运行。2026-07-21 19:00 的 J3-R–NUAA 首次启动因初始化校验的设备不一致失败，失败输出已保留且未自动重跑；校验代码已修正，避免后续 J3-R 任务重复失败。严禁用占位值冒充实验结果。
...
|---|---|---|---|---|---|
| NUAA-SIRST | **已完成1000 epoch**（GPU 1） | GPU 2运行 | GPU 3运行 | GPU 4运行 | GPU 0重跑运行中，best epoch 169 |
| IRSTD-1K | GPU 6运行，尚未开始评估 | 待启动 | 待启动 | 待启动 | 待启动 |
| NUDT-SIRST | 待启动 | 待启动 | 待启动 | 待启动 | 待启动 |

状态更新时间：`2026-07-21 23:14`（Asia/Shanghai）。队列PID `3889915` 正常存活；当前5项运行、1项完成、9项排队，另保留J3-R–NUAA的首次失败归档。J3-R重跑已于22:32在GPU 0启动。运行时权重不提交GitHub。
...
下列 E1/J0 是既有基线，不是本轮重复训练结果。J1–J3 的数值为截至 `2026-07-21 23:14` 从正式 `best_metrics.json` 读取的阶段性best；除NUAA-J1外，其余运行项尚未完成1000 epoch，不能作为最终排名。
...
| NUAA | J2-R | 658（阶段） | 0.7856 | 0.7956 | 0.8799 | 0.9618 | **1.825e-5** |
| NUAA | J2-D | 738（阶段） | 0.7882 | 0.7953 | 0.8816 | 0.9733 | 2.133e-5 |
| NUAA | J3-F | 568（阶段） | 0.7944 | 0.7941 | 0.8854 | **0.9771** | 3.608e-5 |
| NUAA | J3-R | 169（阶段） | 0.7377 | 0.7663 | 0.8491 | 0.9542 | 3.924e-5 |
...
当前NUAA-J1已经完成1000 epoch，最终best为 mIoU/nIoU/F1/Pd/Fa `0.7962/0.8004/0.8865/0.9733/1.955e-5`，相对E1/J0的mIoU提升 `0.0119`、Fa下降。J3-F的阶段best mIoU `0.7944` 接近J1，J2-D为 `0.7882`；J3-R重跑当前处于早期阶段，尚不能进行最终比较。IRSTD-J1仍在训练，其余任务排队。J3-R首次启动失败来自初始化校验的CPU/CUDA设备不一致，修正已提交，失败证据已独立归档，重跑已在GPU 0启动。其余结论继续标记为“待实验”，不是No-Go。
